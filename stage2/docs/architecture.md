...
| 6 数据预测 | 回归/分类/集成/深度学习/时序；模型管理 | `stage2/predict/`：相对时间轴 + 缺测掩码 + 三项基线（全局均值/季节 naive/四周移动平均）+ 扩展窗口滚动前进评估；滞后特征回归与分位数模型在重构中 | `model.json`、基线评估报告、`/api/forecast` | 对应 sklearn/Spark MLlib 的最小可解释实现 |
