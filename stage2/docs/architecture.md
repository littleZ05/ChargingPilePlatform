...
| 3 数据存储 | ODS/DWD/DWS/ADS 分层；HDFS/Hive/HBase 等 | `stage2/warehouse/load.py`：四层装载进 SQLite 数仓，并建模 DWS | `warehouse.db`（41 张表）、`数据字典.md` | HDFS 分区目录、Hive/ClickHouse 表、Iceberg 快照（manifest） |
...
| 5 数据可视化 | ECharts/Grafana/Superset；多维报表与钻取 | `stage2/dashboard/`：离线 ECharts 大屏，三个视图（运营概览/电池样本/数据质量）、筛选联动、钻取弹窗、CSV/JSON 导出 | 大屏页面、`/api/report` 报表 | ECharts（在线）+ 本地离线库 |
| 6 数据预测 | 回归/分类/集成/深度学习/时序；模型管理 | `stage2/predict/`：季节基线 + 线性回归，留一星期交叉验证，模型文件与评估报告，`/api/forecast` | `model.json`、`预测评估.md` | 对应 sklearn/Spark MLlib 的最小可解释实现 |
...
| Atlas / Hive Metastore | `manifest.json` + `数据字典.md` | 元数据、血缘（输入/输出 SHA256）与字段口径 |
