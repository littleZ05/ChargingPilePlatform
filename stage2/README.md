...
| 6 预测（季节基线 + 多跨度岭回归） | `predict/train_v2.py`（主入口）、`predict/train.py`（旧版，待 P4 切换后移除） | `python3 stage2/predict/train_v2.py --db warehouse.db --out model_v2.json --report 预测评估_v2.md` |
...
python3 -m unittest discover -s stage2/predict/tests -v      # 48 项（时间轴/特征/基线/模型/评估/训练 + 既有预测）
